    ...
    User.findOne({ login: login }).exec(function (err, user) {
      ...
      if (err) {
        ...
        if (user.password == hash_password) {
          user.token = uuid(); // 每次登录生成一个uuid作为token
          var socketId = sails.sockets.id(req.socket);
          User.update({id: user.id}, {token: user.token, socket: socketId}).exec(function after(err) {
            if (err) {
              rsp.code = 500;
              res.send(rsp);
            } else {
    ...
    User.find({ token: token}).exec(function (err, data) {
      ...
      if (err) {
        ...
        if (data.length != 1) {
          ...
          var user = data[0];
          User.update({id:user.id}, {status: global.status.STATUS_ONLINE, socket:socketId}).exec(function after(err) {
            if (err) {
              rsp.code = 500;
              res.send(rsp);
            } else {
